...
void CInventoryChanger::PostRetrieveMessage(uint32_t* punMsgType, void* pubDest, uint32_t cubDest, uint32_t* pcubMsgSize)
{
	...
	Ranking->set_rank_id(2);
	Ranking->set_wins(150);

	PlayerCommendationInfo* Commendation = Message.mutable_commendation();
	Commendation->set_cmd_friendly(15);
	Commendation->set_cmd_leader(15);
	Commendation->set_cmd_teaching(15);
	...
	else
	{
		...
		static FARPROC g_pMemAlloc = Interfaces::MemAlloc;
